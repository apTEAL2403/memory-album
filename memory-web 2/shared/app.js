...
function buildPhotoTile(photo) {
  ...
  tile.className = `photo-tile reveal size-${size}`;
  if (photo.url) {
  ...
  // 海原テーマのみ：写真20枚に1回ほど、小さめの写真にペリカンが乗る
  photoRenderCount++;
  if (document.body.dataset.template === "towel" && size === "small" && photoRenderCount % 20 === 0) {
    const pelican = document.createElement("div");
    pelican.className = "pelican-on-photo";
    tile.appendChild(pelican);
  }
...
function buildNoteBlock(note) {
  const el = document.createElement("div");
  el.className = "gallery-item note-block reveal";
  el.innerHTML = `
...
function openLightbox(photo) {
  ...
  document.getElementById("lightbox-meta").innerHTML = `
    <div class="lightbox-actions">
      <button class="download-photo-btn" id="lightbox-download-btn">⬇ 保存</button>
  ...
  `;
  document.getElementById("lightbox-download-btn").addEventListener("click", () => downloadPhoto(photo));
...
// ------------------------------------------------------------
// 写真の保存（スマホでは共有シートから「写真に保存」できるようにする）
// ------------------------------------------------------------
async function downloadPhoto(photo) {
  if (!photo.url) { showToast("画像がありません"); return; }
  showToast("保存の準備をしています...");
  try {
    const res = await fetch(photo.url);
    const blob = await res.blob();
    const ext = (photo.media_type === "video") ? "mp4" : (blob.type.split("/")[1] || "jpg");
    const filename = `${photo.photo_date || "photo"}-${photo.id.slice(0, 8)}.${ext}`;
