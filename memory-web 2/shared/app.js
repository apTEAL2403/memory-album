...
async function loadYears() {
  const [{ data: photoYears }, { data: noteYears }, { data: playlistYears }] = await Promise.all([
...
function renderGallery() {
  ...
  let buffer = [];
  const flushBuffer = () => {
    if (buffer.length) wrap.appendChild(buildDayGroup(buffer));
    buffer = [];
  };

  STATE.items.forEach((item) => {
    if (item.type === "photo") {
      if (buffer.length && buffer[buffer.length - 1].photo_date !== item.data.photo_date) flushBuffer();
...
function buildDayGroup(photos) {
  const el = document.createElement("div");
  el.className = "gallery-item day-group-wrap";
  const label = document.createElement("div");
  label.className = "day-label";
  label.textContent = formatDateLabel(photos[0].photo_date);
  el.appendChild(label);

  const grid = document.createElement("div");
  grid.className = "day-group";
  photos.forEach((photo) => grid.appendChild(buildPhotoTile(photo)));
  el.appendChild(grid);
